...
namespace  Overlay_information
{
    internal  class  Program
    {
        ...
        private  static  void  Main ()
        {
            ...
            PrintEncolored ("Useless w8 4 test some. 500ms", ConsoleColor.Cyan); //
            Thread.Sleep (500) /////////////////////////////////////////// ////////
        ...
        private  static  void  Drawing_OnEndScene (EventArgs  args)
        {
            ...
            DrawButton (_drawHelper.MenuPos.X + 5, + _drawHelper.MenuPos.Y 25, 50, 20, 1, ref ShowCooldownOnTopPanel,
                true,
                new Color (100, 255, 0, 50), new Color (100, 0, 0, 50), "Show Cooldown panel on top");
            //-----------------------------------------------------------------------------------------------------------------
            ...
            DrawButton (_drawHelper.MenuPos.X + 5, + _drawHelper.MenuPos.Y 75, 50, 20, 1, ref ShowHealthOnTopPanel,
                true,
                new Color (100, 255, 0, 50), new Color (100, 0, 0, 50), "Show Health panel on top");
            //-----------------------------------------------------------------------------------------------------------------
            DrawButton (_drawHelper.MenuPos.X + 5, _drawHelper.MenuPos.Y + 100, 50, 20, 1, ref ShowManaOnTopPanel,
                true,
                new Color (100, 255, 0, 50), new Color (100, 0, 0, 50), "Show Mana panel on top");
            //-----------------------------------------------------------------------------------------------------------------
            DrawButton (_drawHelper.MenuPos.X + 5, _drawHelper.MenuPos.Y + 125, 50, 20, 1, ref OverlayOnlyOnEnemy,
                true,
                new Color (100, 255, 0, 50), new Color (100, 0, 0, 50), "Overlay enemy only on");
            //-----------------------------------------------------------------------------------------------------------------
            ...
            if (AutoItemsMenu)
            {
                ...
                DrawButton (_drawHelper.MenuPos.X - 50, _drawHelper.MenuPos.Y + 350, 50, 20, 1, ref AutoItemsPhase,
                    true,
                    new Color (100, 255, 0, 50), new Color (100, 0, 0, 50), "Auto use boots phase");
                //-----------------------------------------------------------------------------------------------------------------
                DrawButton (_drawHelper.MenuPos.X - 50, _drawHelper.MenuPos.Y + 375, 50, 20, 1, ref AutoItemsMidas,
                    true,
                    new Color (100, 255, 0, 50), new Color (100, 0, 0, 50), "Auto midas use");
                ...
                foreach (var t in dummy)
                {
                    for (var n = 0; n <= 4; n ++)
                    {
                        try
                        {
                            ...
                            {
                                = t.AddParticleEffect effect (@ "particles \ ui_mouseactions \ range_display.vpcf");
                        ...
                        if (ShowMeMore)
                        {
                            switch (v.ClassID)
                            {
                                case ClassID.CDOTA_Unit_Hero_Mirana:
                                    ...
                                    var arrow = ObjectMgr.GetEntities <Unit> (). FirstOrDefault (x => x.ClassID ClassID.CDOTA_BaseNPC && == == x.DayVision 650 / * && x.Team! = _ me.Team * /);
                                        ...
                                        if (spell! = null && spell.Cooldown! = 0)
                                        {
                                            ...
                                            if (cd <880)
                                            {
                                                ...
                                                else  if (cd <720 ||! v.IsAlive)
                                                {
                                                    ...
                                                    {
                                                        ParticleEffect eff;
                                                        if (Eff [z] .TryGetValue (v, eff out))
                                                        {
                                                            eff.Dispose ();
                                                            Eff [z] .Clear ();
                                                        }
                        ...
                        var creep =
                            ObjectMgr
                                .GetEntities <Unit> (
                                        ...
                                        x.IsVisible && _me.Distance2D (x) <= midas.CastRange && x.Team! = _ me.Team);
                        if (creep! = null)
                        {
        ...
        private  static  void  HandleEffect (Unit  unit)
        {
            ...
            if (&& unit.IsAlive unit.IsVisibleToEnemies)
            {
                ...
                = unit.AddParticleEffect effect2 ("particles / items2_fx / shadow_amulet_active_ground_proj.vpcf");
        ...
        public  static  void  DrawCircle (int  x, int  y, int  radius, int  numSides, int  thickness, Color  color)
        {
            ...
            var count = 0;
            for (float a = 0; a <(float) * Math.PI 2.0 We do; a + = step)
            {
        ...
        private  static  void  DrawButton (float  x, float  y, float  w, float  h, float  px, ref  bool  clicked, bool  isActive,on Color, Color  off, Colorselect)
        {
            if (isActive)
            {
        ...
        /// <Param  name = "w"> </ param>
        /// <Param  name = "h"> </ param>
        /// <Param  name = "px"> width </ param>
        /// <Param  name = "clicked"> bun for togla </ param>
        /// <Param  name = "isActive"> if the button is active </ param>
        /// <Param  name = "on"> color when asset </ param>
        /// <Param  name = "off"> color when offe </ param>
        /// <Param  name = "description"> Description </ param>
        private  static  void  DrawButton (float  x, float  y, float  w, float  h, float  px, ref  bool  clicked, bool  isActive,on Color, Color  off, string  description)
        {
            var isIn CheckMouse = (x, y, w, h);
            if (isActive)
            {
